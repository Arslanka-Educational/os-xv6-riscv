...
void buddy_bit_inverse(char *array, int index) {
...
void bd_print() { // doesn't work properly
...
void *bd_malloc(uint64 nbytes) {
  ...
  buddy_bit_inverse(bd_sizes[k].xor_alloc_buddy, blk_index(k, p));
  for (; k > fk; k--) {
    // split a block at size k and mark one half allocated at size k-1
    // and put the buddy on the free list at size k-1
    char *q = p + BLK_SIZE(k - 1);  // p's buddy
    bit_set(bd_sizes[k].split, blk_index(k, p));
    buddy_bit_inverse(bd_sizes[k - 1].xor_alloc_buddy, blk_index(k - 1, p));
...
void bd_free(void *p) {
  ...
  for (k = size(p); k < MAXSIZE; k++) {
    ...
    buddy_bit_inverse(bd_sizes[k].xor_alloc_buddy, bi);           // free p at size k
    if (buddy_bit_isset(bd_sizes[k].xor_alloc_buddy, buddy)) {  // is buddy allocated?
...
void bd_mark(void *start, void *stop) {
  ...
  for (int k = 0; k < nsizes; k++) {
    ...
    for (; bi < bj; bi++) {
      ...
      buddy_bit_inverse(bd_sizes[k].xor_alloc_buddy, bi);
    }
  }
}
...
int bd_initfree_pair_right(int k, int bi) {
  int buddy = (bi % 2 == 0) ? bi + 1 : bi - 1;
  int free = 0;
  if (buddy_bit_isset(bd_sizes[k].xor_alloc_buddy, bi)) {
...
void bd_init(void *base, void *end) {
  ...
  for (int k = 0; k < nsizes; k++) {
    ...
    sz = sz / 2 > 0 ? sz / 2 : 1;
    bd_sizes[k].xor_alloc_buddy = p;
    memset(bd_sizes[k].xor_alloc_buddy, 0, sz);
    p += sz;
  }
